        ...
        this.setOnScrollListener(new OnScrollListener() {
            ...
            public void onScroll(AbsListView view, int firstVisibleItem, int visibleItemCount, int totalItemCount) {
                if (firstVisibleItem == 0) {
                    View firstVisibleItemView = MyList.this.getChildAt(0);
                    if (firstVisibleItemView != null && firstVisibleItemView.getTop() == 0) {
                        CURRENT = TOP;
                    }
                } else if ((firstVisibleItem + visibleItemCount) == totalItemCount) {
                    View lastVisibleItemView = MyList.this.getChildAt(MyList.this.getChildCount() - 1);
                    if (lastVisibleItemView != null && lastVisibleItemView.getBottom() == MyList.this.getHeight()) {
                        CURRENT = BOTTOM;
                    }
                } else {
                    CURRENT = MOVE;
